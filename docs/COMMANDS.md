...
| Subcommand | Notes |
|------------|-------|
| `track` | `tag:` and/or `alliance_id:` — scrape HTML now, store roster, add to morning tracked set. Optional `from_tag:` remaps diplomacy/track after a rename; also remaps if stored meta tag differs |
| `resync` | Re-scrape tracked alliance pages in **chunks of 5** (Continue button) — Cloudflare `waitUntil` ~30s after each deferred reply. Resolves tags→**alliance id**, then scrapes by id; remaps renames; **vanished** alliances (id gone + page fail) are untracked/archived. In **testing**, Discord remaps/archives are skipped unless `apply_discord:true` |
...
| `sync_all:` `[plan:]` `[soft_limit:]` `[create_missing:]` … | Letter-bucket categories, rename/move, archive unlinked; `soft_limit` is persisted |
...
|----------|--------|-------|
| Morning (~06:00) alliance roster | Scrape home alliance; activity for all members; leave candidates if absent | Directory + tracked scrapes; **alliance tag renames** remap diplomacy/tracked/DB tags; activity for scraped members; absent verified → live lookup (not auto-guest); diplomacy auto-rebalance if buckets overflow |
