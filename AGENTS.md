...
`wrangler.json` is **gitignored** and generated by `npm run generate-config`. DO migrations use `new_sqlite_classes` (Free-plan / new-account requirement). Do not rewrite already-applied legacy `new_classes` tags on existing Workers.
