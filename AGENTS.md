...
- `/server verify-panel` — channel **Start verification** button; toggles **Invite DM** on join (`verification_invite_mode` `dm`|`channel_panel`); demotion notify `dm`|`channel`|`none` (separate from post-verify `/server welcome`)
...
- `/alliance track|suggest|list|untrack|defer-untracked-admirals` (multi) — scrape now + nick-based link suggestions (ASCII table); **Approve 🟢/🟡/🟠** per confidence + **Continue** (Free **2**/click, Paid **6**/click, max **10**) — `src/workers-plan.ts`, `docs/ADMIN_GUIDE.md` § Approve-all chunking; optional deferral of Admiral roles + diplomacy until track
...
src/
  ...
  alliance-handlers.ts     # /alliance track|suggest + Approve/Continue button chunks
  link-suggest.ts          # Nick parse + Discord↔roster match suggestions
  workers-plan.ts          # WORKERS_PLAN free/paid → Approve-all chunk size
...
| `WORKERS_PLAN` | `free` (default) \| `paid` — Approve-all chunk size; Paid enables `limits.cpu_ms` |
| `ALLIANCE_APPROVE_CHUNK` | Optional override (1–10 hard max) for links per Approve-all click |
